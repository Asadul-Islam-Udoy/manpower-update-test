...
import { delteAdminAction, getAllAdminListsAction, refreshAuthAction } from "../../action/auth_admin/AdminAction";
function AdminDeleteModal({ adminId, name }) {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const { error, lodding, isAdminDelete } = useSelector(
  ...
  }, [isAdminDelete, dispatch, error, toast]);
  return (
    <>
      <Dialog
        open={open}
      ...
      >
        <div class="relative w-full max-w-2xl">
          <div
            ...
            class="inline-block w-full max-w-md overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl xl:max-w-xl"
          >
            <div className="border-b-2">
              <div class="flex items-center justify-between space-x-4 px-4 py-3 bg-slate-200">
                <h1 class="text-xl font-bold text-gray-800 flex items-center">
                  <span>Are you sure delete? <b className=" text-blue-400">{name}</b></span>
                </h1>
                <button
                  type="button"
            ...
            <svg
              class="h-32 w-32 mx-auto rounded-full p-3 bg-red-200 mt-4 text-red-600"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
            ...
            <p
              style={{
                color:
                  theme.palette.mode === "dark" ? "white" : "rgb(18, 48, 85)",
              }}
              class="text-xl text-gray-800 p-4 text-center"
            >
