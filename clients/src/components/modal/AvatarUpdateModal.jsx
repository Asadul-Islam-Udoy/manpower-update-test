...
function AvatarUpdateModal({ id }) {
  ...
  const { error, lodding, isAvatarUpdate } = useSelector(
  ...
  const [shwoAvator, setShowAvatar] = useState(singleworker?.worker?.avatar);
  useEffect(() => {
    if (error) {
      toast.error(error);
    }
  ...
  }, [id, singleError, toast, isAvatarUpdate, navigate, error]);
  ...
  useEffect(() => {
    setShowAvatar(singleworker?.worker?.avatar);
  }, [singleworker]);
              ...
              {avatar ? (
                ...
                <Typography>
                  ...
                  <Link
                    ...
                    to={Localhost + `/images/avatars/${shwoAvator}`}
                  >
