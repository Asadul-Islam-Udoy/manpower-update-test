...
import { ArupResumeAction} from "../../action/auth_user/UserAction";
function ArupResumeModal({ workerId}) {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const { error, isArupResume } =
  useSelector((state) => state.workerResumeApplyState);
  ...
  const resumeArupHandler = (id) => {
    setButtonDisabled(true);
    dispatch(ArupResumeAction(id));
  };

 useEffect(() => {
    if (isArupResume) {
      toast.success("worker resume arup successfully");
      setOpen(false);
  ...
  }, [isArupResume, dispatch, toast,error]);
  return (
    <>
      <Dialog
        open={open}
        ...
        <div class="relative w-full max-w-2xl">
              <div class="inline-block w-full max-w-md overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl xl:max-w-xl">
                <div className="border-b-2">
                  <div class="flex items-center justify-between space-x-4 px-4 py-3 bg-slate-200">
                    <h1 class="text-xl font-bold text-gray-800 flex items-center">
                      ...
                      <span>Are you sure arup this resume?</span>
                    </h1>
                    <button
                      type="button"
                ...
                <p class="text-xl text-gray-800 p-4 text-center">
                  If you continue, you will permanently arup this record. Are
                ...
                <div class="flex justify-end items-center space-x-4  px-4 py-2 bg-slate-200">
                  ...
                  <button
                    onClick={() => resumeArupHandler(workerId)}
                    autoFocus
...
export default ArupResumeModal;
